...
type RowStatus = 'idle' | 'snoozing' | 'snoozed';
  ...
  if (status === 'snoozed') {
    return (
      <tr className="opacity-40 border-b bg-green-50">
        ...
        <td colSpan={3} className="px-3 py-2 text-sm text-green-600">✓ 7日間スヌーズ</td>
      </tr>
    );
  }
  ...
  return (
    <tr className="border-b hover:bg-gray-50">
      ...
      <td className="px-3 py-2 whitespace-nowrap">
        <button onClick={handleSnooze} disabled={status === 'snoozing'} className="px-3 py-1 bg-gray-100 text-sm rounded hover:bg-gray-200 disabled:opacity-50">
          {status === 'snoozing' ? '処理中...' : '確認済み'}
        </button>
      </td>
    </tr>
  );
